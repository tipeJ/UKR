...
** TODO Image fetching for movies (TMDB, etc.)
   Currently doesn't work, because there is no API key. Look into kodi cached images before getting API keys for different fetching websites. Ideally everything in local network.
